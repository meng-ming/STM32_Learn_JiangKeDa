...
PWM_LED_t PWM_LED_Param_Init = {0};

void Calcuelate_Param_Init(uint16_t PWM_Frequency,uint16_t PWM_Resolution,uint8_t PWM_Duty_Ratio)
{
	PWM_LED_Param_Init.ARR = 100 / PWM_Resolution - 1;
	PWM_LED_Param_Init.PSC = (72000000 / PWM_Frequency / (PWM_LED_Param_Init.ARR + 1)) - 1;;
	PWM_LED_Param_Init.CCR = PWM_Duty_Ratio * (PWM_LED_Param_Init.ARR + 1) / 100;
}
...
void PWM_LED_Init(void)
{
	...
	// GPIO_PinRemapConfig(GPIO_PartialRemap1_TIM2,ENABLE); // TIM2_CH1 重映射端口在 PA15
	// GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable,ENABLE); // 关闭调试端口
